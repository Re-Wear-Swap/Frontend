...
export function LoginPage() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    login({ name: email.split('@')[0], username: email.split('@')[0], email, photo: null, points: 1, swaps: 0, verified: true })
    navigate('/profile')
  }
  ...
  return (
    <HomeTemplate activeTab="perfil">
      <div style={{ padding: '24px 20px' }}>
        <h1 style={{ fontSize: 28, fontWeight: 800, marginBottom: 10, color: text }}>
          ¡Bienvenid@ de nuevo!
        </h1>
        <p style={{ fontSize: 16, marginBottom: 20, color: text }}>
        ...
        <p style={{ marginTop: 20, fontSize: 14, color: text }}>
          ...
          <span onClick={() => navigate('/register')} style={{ color: '#9333ea', fontWeight: 700, cursor: 'pointer' }}>
            Regístrate aquí.
          </span>
        </p>
      </div>
